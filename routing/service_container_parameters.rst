...
Você também pode usar um parâmetro para definir o seu path da rota (ou parte do seu
path):
...
.. note::
    ...
    No entanto, como os caracteres ``%`` incluídos em qualquer URL são automaticamente codificados,
    a URL resultante desse exemplo seria ``/score-50%25`` (``%25`` é o
    resultado da codificação do caractere ``%``).
